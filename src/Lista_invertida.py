...
stemmer_string = configs.get('GLI.CFG', 'STEMMER')
...
for caminho_arquivo in arquivos:
    ...
    for artigo in root.findall('RECORD'):
        ...
        if abstract is not None and len(abstract.text) > 0:
            ...
            if stemmer:
                abstract_palavras = [ps.stem(p).upper() for p in abstract_palavras]
            ...
            if stemmer:
                extract_palavras = [ps.stem(p).upper() for p in extract_palavras]
